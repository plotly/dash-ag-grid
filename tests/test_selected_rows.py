...
def test_sr4_selected_rows_get_row_id_bracket_notation(dash_duo):
    app = Dash(__name__)

    row_data = [
        {"Current canvas blend": "alpha", "B": 1},
        {"Current canvas blend": "O'Reilly", "B": 2},
        {"Current canvas blend": r"path\\name", "B": 3},
    ]

    app.layout = html.Div([
        html.Button('Select Special', id='select-special', n_clicks=0),
        dag.AgGrid(
            id='grid',
            columnSize="sizeToFit",
            columnDefs=[{'field': 'Current canvas blend'}, {'field': 'B'}],
            rowData=row_data,
            getRowId='params.data["Current canvas blend"]',
            dashGridOptions={"rowSelection": "multiple"},
            style={"maxHeight": "200px", "overflow": "auto"},
        ),
        html.Div(id='selected-row-info')
    ])

    @app.callback(
        Output('grid', 'selectedRows'),
        Input('select-special', 'n_clicks'),
        prevent_initial_call=True
    )
    def select_special_row(n_clicks):
        return [{"Current canvas blend": "O'Reilly", "B": 2}]
    ...
    def update_selected_row_info(selected_rows):
        if isinstance(selected_rows, list) and selected_rows:
            return f"Selected Row: {selected_rows[-1]['Current canvas blend']}"
        return "No row selected"

    dash_duo.start_server(app)

    grid = utils.Grid(dash_duo, "grid")
    grid.wait_for_cell_text(0, 0, "alpha")

    dash_duo.find_element('#select-special').click()
    dash_duo.wait_for_text_to_equal(
        "#selected-row-info", "Selected Row: O'Reilly"
    )
