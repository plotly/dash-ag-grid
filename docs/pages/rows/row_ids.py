...
Each grid row has a unique row ID which is used by the grid to identify rows, e.g. for identifying what rows are
currently selected. These row IDs can be Grid Assigned or Application Assigned.
...
even if the second set of rows are logically identical to the first set of rows (i.e. it's a refresh operation).

When Row Grouping another zero based sequence is used and prefixed with `row-group-`. For example Row Groups will have
...
When the Application assigns Row IDs, this provides a common Row ID across the grid and the Application. This has
advantages such as:

When updating Row Data (either by updating the `rowData` attribute, or using Update Transactions), the grid uses the Row
ID to map old vs new data (e.g. it can work out which Rows to add / remove / update rather than doing a blanket
replacement of old vs new data). This allows the grid to keep state such as Row Selection.
...
In this example, the data contains a unique id in the dataset in the "id" column. We use `getRowId="params.data.id"` to
assign the id.
